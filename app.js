...
{ key: “thumb4”, time: “1:37”, pct: 95, label: “Current”, active: true }
]
};
...
/* ── プレイヤー画像（デフォルト: thumb1）── */
function applyPlayerImage() {
const frame = document.getElementById(‘playerFrame’);
if (!frame) return;
// デフォルトはthumb1（タイムスタンプ最初のフレーム）
const src = cfg.images && (cfg.images.thumb1 || cfg.images.player);
if (!src) return;

applyPlayerImageSrc(src);
}
...
// プレイヤー画像をそのフレームの画像に切り替え
const src = thumbImages[idx];
if (src) applyPlayerImageSrc(src);
}
